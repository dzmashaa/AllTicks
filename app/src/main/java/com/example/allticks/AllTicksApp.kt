    ...
    Scaffold(
        snackbarHost = {
          SnackbarHost(
              ...
              modifier = Modifier.padding(16.dp).imePadding().navigationBarsPadding(),
              snackbar = { snackbarData ->
                Snackbar(
                    snackbarData, containerColor = primaryLight, shape = RoundedCornerShape(8.dp))
              })
        },
        bottomBar = {
          if (getBottomBarScreens().any { it.route == currentRoute }) {
            BottomNavigationBar(navController = appState.navController, appState = appState)
          }
...
fun rememberAppState(
    ...
    remember(snackbarHostState, navController, snackbarManager, resources, coroutineScope) {
      AllTicksAppState(snackbarHostState, navController, snackbarManager, resources, coroutineScope)
    }
...
fun NavGraphBuilder.allTicksGraph(appState: AllTicksAppState) {
  ...
  composable(CALENDAR_SCREEN) { CalendarScreen(openScreen = { route -> appState.navigate(route) }) }
  composable(
      ...
      arguments =
        ...
        EditTaskScreen(dueDate = dueDate, popUpScreen = { appState.popUp() })
      }
}
